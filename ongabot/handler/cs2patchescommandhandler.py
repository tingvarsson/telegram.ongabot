...
async def _post_latest_patch_note(context: CallbackContext, chat_id: int) -> None:
    """Post the most recent CS2 patch note to one chat, so turning the feature on shows it.

    A one-off for this chat only: it neither reads nor updates BotData.cs2_patchnotes_seen_gids,
    which belongs to the sweep job's broadcast to every subscriber.
    """
...
async def callback(update: Update, context: CallbackContext) -> None:
    ...
    await _post_latest_patch_note(context, chat_id)
